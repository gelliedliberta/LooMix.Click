...
class Tag extends Model {
     ...
     */
    public function getActiveTags($limit = null) {
        $sql = "SELECT * FROM {$this->table} WHERE is_active = 1 ORDER BY usage_count DESC, name ASC";
    ...
    public function updateUsageCount($tagId) {
        ...
        return $this->db->query($sql, ['tag_id' => $tagId]);
    }
    ...
    public function getRelatedTags($tagId, $limit = 10) {
        $sql = "SELECT t2.*, COUNT(*) as relation_count 
                ...
                WHERE t1.id = :tag_id AND t2.id != :tag_id 
